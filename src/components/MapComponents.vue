<template>
  <!-- <pre>{{ markers }}</pre> -->
  <div ref="cmaps" class="bg-grey-1" :class="size"></div>
  ...
  methods: {
          ...
          case "google.maps":
            ...
            if (typeof this.cmaps !== "undefined" && this.cmaps !== null) {
              ...
              this.cmaps = new window.google.maps.Map(this.$refs.cmaps, {
                ...
                styles: this.map_style ? JSON.parse(this.map_style) : "",
              });
            }
    ...
    addMarker(properties, index, draggable) {
      try {
        switch (this.provider) {
          case "google.maps":
            if (Array.isArray(cmapsMarker) && cmapsMarker[index]) {
