      ...
      >
        ...
        >
          <q-btn
            no-caps
            unelevated
            :color="!client_secret ? 'disabled' : 'primary'"
...
export default {
  ...
  methods: {
    ...
    showPaymentForm(value) {
      this.credentials = value ?? this.payment_credentials;
      this.modal = true;
      this.createCustomer();
    },
    async createCustomer() {
      try {
        ...
        const results = await APIinterface.PaymentPost(
    ...
    async savePayment(value) {
      ...
      try {
        await APIinterface.PaymentPost(
          STRIPE_ENDPOINT + "/StripeSavePayment",
          params,
          true
        );
        this.modal = false;
    ...
    async PaymentRender(value) {
      try {
        APIinterface.showLoadingBox(this.$t("Processing payment"), this.$q);
        const params = {
        ...
        const results = await APIinterface.PaymentPost(
          STRIPE_ENDPOINT + "/StripePaymentIntent",
          params,
          true
        );
    ...
    async Dopayment(data) {
      try {
        APIinterface.showLoadingBox("", this.$q);
        const results = await APIinterface.PaymentPost(
          STRIPE_ENDPOINT + "/StripeProcesspayment",
          {
            data: data.data,
          },
          true
