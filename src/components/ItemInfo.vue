...
export default {
  ...
  methods: {
    ...
    makeAnother() {
      ...
      this.$emit("showItemdetails", item.cat_id, item.item_token, null);
      this.modal = false;
    },
    async removeItem(value) {
      try {
        ...
        await APIinterface.fetchDataPost("removeCartItem", params);
    ...
    async updateCartQty(Qty, itemQty, item) {
      ...
      try {
        ...
        const params = new URLSearchParams({
          cart_uuid: this.cart_uuid,
          cart_row: item.cart_row,
          item_qty: QtyTotal,
        }).toString();
        await APIinterface.fetchDataPost("updateCartItems", params);
        this.$emit("afterUpdateqty");
      } catch (error) {
