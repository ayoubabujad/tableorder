<template>
  <div class="relative-position">
    <q-list v-if="getBalance > 0">
      <q-item tag="label">
        <q-item-section avatar class="min-avatar" top>
          <q-icon name="o_wallet" color="blue-grey-6"></q-icon>
        </q-item-section>
        <q-item-section>
          ...
          <q-item-label caption v-if="message">
            <div class="q-pa-sm radius8 bg-grey-2 text-blue-grey-8">
              {{ message }}
            </div>
          </q-item-label>
        </q-item-section>
    ...
    async applyDigitalWallet(data) {
      try {
        ...
        const params = new URLSearchParams({
          ...
          amount_to_pay: this.CartStore.getTotalRaw,
        }).toString();
        ...
        this.$emit("afterApplywallet", response.details);
      } catch (error) {
