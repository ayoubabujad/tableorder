...
export default {
  ...
  methods: {
    ...
    Dopayment(value) {
      ...
      let redirect = data.payment_url + "?data=" + value + "&app=1";
    ...
    Webview(url) {
      ...
      inapp.addEventListener("loadstop", (event) => {
        ...
        let res = even_url.match(/successful/gi);
        if (!APIinterface.empty(res)) {
          console.log("succesful");
          inapp.close();
          this.$emit("afterSuccessfulpayment", this.params);
        }
