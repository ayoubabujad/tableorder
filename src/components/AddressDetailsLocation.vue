<template>
  ...
  >
    <q-card>
      ...
      <div
        class="fixed-bottom q-pa-sm footer-shadow row q-gutter-x-md items-center my-z-top"
        :class="{
          'bg-dark': $q.dark.mode,
          'bg-white': !$q.dark.mode,
        }"
  ...
  methods: {
    ...
    async beforeShow() {
      ...
      if (!latitude) {
        ...
        try {
          if (this.$q.capacitor) {
            coordinate_response = await this.DataStore.fetchLocation(this.$t);
          } else {
            coordinate_response = await this.DataStore.fetchWebLocation(
              this.$t
            );
          }
          if (location) {
    ...
    async onSubmit() {
      ...
      try {
        ...
        this.loading = true;
        const response = await APIinterface.PaymentPost(
