<template>
  ...
  <q-page class="flex items-stretch content-end q-pa-md bg-grey-1x">
      ...
      <template v-for="items in getChatmessage" :key="items">
        ...
        <template v-else>
          <q-chat-message
            ...
            :avatar="items.photo"
            :stamp="items.time"
            :text-color="items.senderID == user_uuid ? 'dark' : 'white'"
            :bg-color="items.senderID == user_uuid ? 'grey-1' : 'blue-8'"
    ...
    <q-footer
      ...
      class="bg-white q-pl-md q-pr-md q-pb-sm text-text"
      :class="{
        'footer-shadow': is_recording,
      }"
    >
      ...
      <template v-else>
        ...
        <div class="row q-gutter-x-sm">
          <q-input
            ...
            outlined
            autogrow
            class="col border-grey1 radius8"
            color="transparent"
          >
...
export default {
  ...
  created() {
    this.chat_api_url = this.chat_api_url.replace("/partnerapi/", "/");
    this.upload_api_url = this.chat_api_url + "/uploadimage";
  },
  mounted() {
    this.user_data = auth.getUser();
    this.user_uuid = this.user_data.client_uuid;
    ...
    if (!APIinterface.empty(added)) {
      this.DataStore.data.chat_data = null;
    }
    this.getMessages();
    this.getParticipant();
    this.getWhoIsTyping();
    ...
    async getParticipant() {
      try {
        ...
        if (dataSnapshot.exists()) {
          ...
          this.page_title = `${this.getChatWith?.first_name} ${this.getChatWith?.last_name}`;
          this.page_avatar = this.getChatWith?.avatar;
